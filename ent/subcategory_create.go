...
func (scc *SubCategoryCreate) SetTitle(s string) *SubCategoryCreate {
	scc.mutation.SetTitle(s)
	return scc
}
...
func (scc *SubCategoryCreate) check() error {
	if _, ok := scc.mutation.Title(); !ok {
		return &ValidationError{Name: "title", err: errors.New(`ent: missing required field "SubCategory.title"`)}
	}
...
func (scc *SubCategoryCreate) createSpec() (*SubCategory, *sqlgraph.CreateSpec) {
	...
	if value, ok := scc.mutation.Title(); ok {

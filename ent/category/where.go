...
// HasSubs applies the HasEdge predicate on the "subs" edge.
func HasSubs() predicate.Category {
	return predicate.Category(func(s *sql.Selector) {
		step := sqlgraph.NewStep(
			sqlgraph.From(Table, FieldID),
			sqlgraph.To(SubsTable, FieldID),
			sqlgraph.Edge(sqlgraph.O2M, false, SubsTable, SubsColumn),
		)
		sqlgraph.HasNeighbors(s, step)
	})
}

// HasSubsWith applies the HasEdge predicate on the "subs" edge with a given conditions (other predicates).
func HasSubsWith(preds ...predicate.SubCategory) predicate.Category {
	return predicate.Category(func(s *sql.Selector) {
		step := sqlgraph.NewStep(
			sqlgraph.From(Table, FieldID),
			sqlgraph.To(SubsInverseTable, FieldID),
			sqlgraph.Edge(sqlgraph.O2M, false, SubsTable, SubsColumn),
		)

...
// QuerySubs chains the current query on the "subs" edge.
func (cq *CategoryQuery) QuerySubs() *SubCategoryQuery {
...
// WithSubs tells the query-builder to eager-load the nodes that are connected to
// the "subs" edge. The optional arguments are used to configure the query builder of the edge.
func (cq *CategoryQuery) WithSubs(opts ...func(*SubCategoryQuery)) *CategoryQuery {
	query := &SubCategoryQuery{config: cq.config}
...
func (cq *CategoryQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Category, error) {
	...
	if query := cq.withSubs; query != nil {
		if err := cq.loadSubs(ctx, query, nodes,
			func(n *Category) { n.Edges.Subs = []*SubCategory{} },
			func(n *Category, e *SubCategory) { n.Edges.Subs = append(n.Edges.Subs, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (cq *CategoryQuery) loadSubs(ctx context.Context, query *SubCategoryQuery, nodes []*Category, init func(*Category), assign func(*Category, *SubCategory)) error {
	...
	query.Where(predicate.SubCategory(func(s *sql.Selector) {
		s.Where(sql.InValues(category.SubsColumn, fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.category_subs
		if fk == nil {
			return fmt.Errorf(`foreign-key "category_subs" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "category_subs" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil

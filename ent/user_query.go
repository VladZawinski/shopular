...
func (uq *UserQuery) loadCarts(ctx context.Context, query *CartQuery, nodes []*User, init func(*User), assign func(*User, *Cart)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(user.CartsTable)
		s.Join(joinT).On(s.C(cart.FieldID), joinT.C(user.CartsPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(user.CartsPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(user.CartsPrimaryKey[0]))
	...
	for _, n := range neighbors {
		nodes, ok := nids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected "carts" node returned %v`, n.ID)
		}
		for kn := range nodes {
			assign(kn, n)
		}
